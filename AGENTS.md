...
- `src/config/database.ts` registers entities + 12 migrations (users/audit → catalog → questions → old assessments → old rename → old roadmaps → adaptive → new assessments → content-imports → new rename → roadmaps-AI-columns → drop-legacy-assessments). `src/database/migrations/` is the versioned schema; `DB_SCHEMA.txt` mirrors the initial design only, not all migrations.
