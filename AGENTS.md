...
- `src/config/database.ts` registers entities + 6 migrations (users/audit → catalog → questions → assessments → rename user answers → assessment guards). `src/database/migrations/` is the versioned schema; `DB_SCHEMA.txt` documents catalog, questions, and assessments.
